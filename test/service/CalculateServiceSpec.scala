...
"Calculate service " should {
  ...
  "return correct sum of 2.05 if list entered has mix of upper and lower case" in {
    ...
    val result = calculateService.calculate(List("Apple","Apple","Orange","Apple"))
    val expected = 2.05
  ...
  "return correct sum of 0.00 if list entered is empty" in {
    ...
    val result = calculateService.calculate(List())
    val expected = 0.00
    expected mustBe(result)
  }
  ...
  "return -2 if the list entered has item not in the approved list" in {
    ...
    val result = calculateService.calculate(List("Apple", "Lemon"))
    val expected = -2
    expected mustBe(result)
  }
}
